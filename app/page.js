...
async function doExportReport(prop, data) {
  ...
  // === 7. REGULATORY OUTCOME ANALYSIS ===
  if (data.regulatoryOutcome) {
    const ro = data.regulatoryOutcome;
    sectionTitle("7", "Regulatory Outcome Analysis");
    if (ro.hasPendingRegulation && ro.pendingItems && ro.pendingItems.length > 0) {
      ro.pendingItems.forEach(item => {
        checkPage(40);
        doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.setTextColor(15, 23, 42);
        doc.text(item.name || "", M, y); y += 6;
        const halfW = (CW - 4) / 2;
        // IF PASSES column
        doc.setFillColor(254, 242, 242); doc.rect(M, y, halfW, 5, "F");
        doc.setFontSize(7.5); doc.setFont("helvetica", "bold"); doc.setTextColor(153, 27, 27);
        doc.text("IF PASSES", M + 3, y + 3.5); y += 6;
        const passFields = [
          ["CAP", item.ifPasses?.cap], ["BASE RENT", item.ifPasses?.baseRent],
          ["VACANCY DECONTROL", item.ifPasses?.vacancyDecontrol],
          ["EFFECTIVE DATE", item.ifPasses?.effectiveDate], ["EXEMPTIONS", item.ifPasses?.exemptions]
        ].filter(f => f[1]);
        const passStartY = y;
        passFields.forEach(([lbl, val]) => {
          doc.setFontSize(7); doc.setFont("helvetica", "bold"); doc.setTextColor(100, 116, 139);
          doc.text(lbl, M + 3, y);
          doc.setFontSize(8.5); doc.setFont("helvetica", "normal"); doc.setTextColor(30, 41, 59);
          const vLines = doc.splitTextToSize(String(val), halfW - 8);
          y += 3.5; doc.text(vLines, M + 3, y); y += vLines.length * 3.5 + 2;
        });
        const passEndY = y;
        // IF FAILS column
        y = passStartY - 6;
        const rx = M + halfW + 4;
        doc.setFillColor(240, 253, 244); doc.rect(rx, y, halfW, 5, "F");
        doc.setFontSize(7.5); doc.setFont("helvetica", "bold"); doc.setTextColor(22, 101, 52);
        doc.text("IF FAILS / STATUS QUO", rx + 3, y + 3.5); y += 6;
        const failFields = [
          ["CAP", item.ifFails?.cap], ["SUMMARY", item.ifFails?.summary]
        ].filter(f => f[1]);
        failFields.forEach(([lbl, val]) => {
          doc.setFontSize(7); doc.setFont("helvetica", "bold"); doc.setTextColor(100, 116, 139);
          doc.text(lbl, rx + 3, y);
          doc.setFontSize(8.5); doc.setFont("helvetica", "normal"); doc.setTextColor(30, 41, 59);
          const vLines = doc.splitTextToSize(String(val), halfW - 8);
          y += 3.5; doc.text(vLines, rx + 3, y); y += vLines.length * 3.5 + 2;
        });
        y = Math.max(y, passEndY) + 2;
        // Polling & status tags
        if (item.polling || item.status) {
          doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(71, 85, 105);
          const tags = [];
          if (item.polling) tags.push("Polling: " + item.polling);
          if (item.status) tags.push("Status: " + item.status);
          const tagLine = doc.splitTextToSize(tags.join("  |  "), CW);
          doc.text(tagLine, M, y); y += tagLine.length * 3.5 + 4;
        }
      });
    } else {
      checkPage(14);
      doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(148, 163, 184);
      doc.text("N/A", M + CW / 2 - 5, y); y += 5;
      doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(148, 163, 184);
      const naLines = doc.splitTextToSize(ro.naReason || "No pending regulatory changes identified.", CW - 20);
      doc.text(naLines, M + 10, y); y += naLines.length * 3.5 + 4;
    }
  }
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        if (!ro) return null;
        return (<>
          <Sec icon="⑧">Regulatory Outcome Analysis</Sec>
          <Card>
            {ro.hasPendingRegulation && ro.pendingItems && ro.pendingItems.length > 0 ? (
              ro.pendingItems.map((item, i) => (
                <div key={i} style={{ marginBottom: i < ro.pendingItems.length - 1 ? 16 : 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", marginBottom: 10 }}>{item.name}</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, border: "1px solid #e2e8f0", borderRadius: 6, overflow: "hidden" }}>
                    <div style={{ background: "#fef2f2", padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                      <div style={{ fontSize: 10.5, fontWeight: 700, color: "#991b1b", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>If Passes</div>
                      {item.ifPasses?.cap && <div style={{ marginBottom: 5 }}><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>CAP</span><div style={{ fontSize: 12.5, color: "#1e293b", fontWeight: 600 }}>{item.ifPasses.cap}</div></div>}
                      {item.ifPasses?.baseRent && <div style={{ marginBottom: 5 }}><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>BASE RENT</span><div style={{ fontSize: 12, color: "#1e293b" }}>{item.ifPasses.baseRent}</div></div>}
                      {item.ifPasses?.vacancyDecontrol && <div style={{ marginBottom: 5 }}><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>VACANCY DECONTROL</span><div style={{ fontSize: 12, color: "#1e293b" }}>{item.ifPasses.vacancyDecontrol}</div></div>}
                      {item.ifPasses?.effectiveDate && <div style={{ marginBottom: 5 }}><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>EFFECTIVE DATE</span><div style={{ fontSize: 12, color: "#1e293b" }}>{item.ifPasses.effectiveDate}</div></div>}
                      {item.ifPasses?.exemptions && <div><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>EXEMPTIONS</span><div style={{ fontSize: 11.5, color: "#1e293b" }}>{item.ifPasses.exemptions}</div></div>}
                    </div>
                    <div style={{ background: "#f0fdf4", padding: "10px 14px" }}>
                      <div style={{ fontSize: 10.5, fontWeight: 700, color: "#166534", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>If Fails / Status Quo</div>
                      {item.ifFails?.cap && <div style={{ marginBottom: 5 }}><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>CAP</span><div style={{ fontSize: 12.5, color: "#1e293b", fontWeight: 600 }}>{item.ifFails.cap}</div></div>}
                      {item.ifFails?.summary && <div><span style={{ fontSize: 10, color: "#64748b", fontWeight: 600 }}>SUMMARY</span><div style={{ fontSize: 12, color: "#1e293b" }}>{item.ifFails.summary}</div></div>}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 12, marginTop: 8, flexWrap: "wrap" }}>
                    {item.polling && <div style={{ background: "#f1f5f9", padding: "4px 10px", borderRadius: 4, fontSize: 11, color: "#475569" }}><strong>Polling:</strong> {item.polling}</div>}
                    {item.status && <div style={{ background: "#f1f5f9", padding: "4px 10px", borderRadius: 4, fontSize: 11, color: "#475569" }}><strong>Status:</strong> {item.status}</div>}
                  </div>
                </div>
              ))
            ) : (
              <div style={{ textAlign: "center", padding: "20px 16px" }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#94a3b8", marginBottom: 6 }}>N/A</div>
                <div style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.6, maxWidth: 480, margin: "0 auto" }}>{ro.naReason || "No pending regulatory changes identified that would materially affect this property."}</div>
              </div>
            )}
          </Card>
        </>);
